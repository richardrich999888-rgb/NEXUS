...
class GovernanceImmuneBridge:
    ...
    def register_agent(self, agent_id: str, permissions: Optional[List[str]] = None) -> Dict:
        """Register an agent for immune-governance tracking."""
        self.agent_permissions[agent_id] = permissions or ["*"]
        self.trust_scores.setdefault(agent_id, 1.0)
    ...
    def quarantine_agent(self, agent_id: str) -> Dict:
        """Compatibility helper for callers that request direct quarantine."""
        signal = ThreatSignal(
            agent_id=agent_id,
            threat_level=ThreatLevel.CRITICAL,
            threat_type="manual_quarantine",
            confidence=1.0,
        )
        return self.register_threat(signal)

    def restrict_agent(self, agent_id: str, permissions: Optional[List[str]] = None) -> Dict:
        """Compatibility helper for restricting an agent's permissions."""
        self.agent_permissions[agent_id] = permissions or ["read"]
        self.update_trust(agent_id, -0.25)
        return {
            "action": "restrict",
            "agent_id": agent_id,
