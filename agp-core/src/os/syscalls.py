...
class SysCallHandler:
    ...
    async def handle_syscall(self, pid: int, syscall_type: SysCallType, args: Dict, token: str = None) -> Dict:
        ...
        try:
            ...
            if syscall_name == 'EXEC':
                ...
                if handler:
                    result = await handler(process, args)
                else:
    ...
    async def _handle_malloc(self, process: ProcessState, args: Dict) -> Dict:
        ...
        if process.usage.memory_mb + (size / (1024 * 1024)) > process.quota_memory:
             return {"error": "ENOMEM (Quota exceeded)"}
             
        process.update_usage(memory=size)
        return {"status": "success", "addr": f"0x{uuid.uuid4().hex[:8]}", "size": size}
    ...
    def _handle_fork(self, process: ProcessState, args: Dict) -> Dict:
        """
