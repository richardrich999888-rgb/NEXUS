...
fn decompose_inferred(payload: &Payload) -> Result<DecompositionResult, EncodeError> {
    ...
    let patterns = find_structural_patterns(bytes);

    // If no patterns found, treat entire payload as variable
...
fn find_structural_patterns(bytes: &[u8]) -> Vec<PatternOccurrence> {
    ...
    for start in 0..len.saturating_sub(MIN_PATTERN_LEN) {
...
mod tests {
    ...
    fn test_decompose_with_patterns() {
        // Payload with repeating pattern
        let data = b"AAAA1234AAAA5678AAAA9012".to_vec();
        let payload = Payload::new(data);

        let result = decompose(&payload).unwrap();

        // Should identify "AAAA" as structural pattern
        assert!(
            result.structure.levels.iter().any(|l| l.literals == b"AAAA"),
            "Should find AAAA pattern"
        );
    }
