...
pub struct RepetitionSpec {
    ...
    pub count: u32,
    /// Byte offset between repetitions
    pub stride: u32,
...
impl Artifact {
    ...
    /// Production systems may use more compact binary format.
    pub fn to_bytes(&self) -> Vec<u8> {
        // Note: serde_json produces deterministic output for these types
        serde_json::to_vec(self).expect("Artifact serialization cannot fail")
    }

    /// Deserialize artifact from bytes.
    pub fn from_bytes(bytes: &[u8]) -> Result<Self, ArtifactError> {
        serde_json::from_slice(bytes).map_err(|e| ArtifactError::DeserializationFailed {
            reason: e.to_string(),
        })
    }
}
