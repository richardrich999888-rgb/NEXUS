...
pub fn fee_encode(structure: &Structure) -> Result<FeeEncodeResult, EncodeError> {
    if structure.levels.is_empty() {
        ...
        return Ok(FeeEncodeResult {
            generator: Generator {
                base: vec![],
                repetition: RepetitionSpec { count: 0, stride: 0, start_offset: 0, byte_ranges: vec![] },
            },
...
/// Stores actual byte_ranges for exact reconstruction (non-uniform spacing support).
fn calculate_repetition(ranges: &[ByteRange], pattern_len: usize) -> RepetitionSpec {
    ...
    // Kept for backward compatibility and compression hints
...
/// Uses stored byte_ranges for exact reconstruction (non-uniform spacing support).
pub fn regenerate_structure(generator: &Generator, mappings: &MappingSet) -> Structure {
    ...
    // Use stored byte_ranges for exact reconstruction
    // Fall back to stride-based calculation for backward compatibility
    let byte_ranges = if !generator.repetition.byte_ranges.is_empty() {
        generator.repetition.byte_ranges.clone()
    } else {
        // Backward compatibility: derive from stride (only for old artifacts)
        let pattern_len = generator.base.len();
        (0..generator.repetition.count)
            .map(|i| {
                let start = generator.repetition.start_offset 
                    + (i as usize * generator.repetition.stride as usize);
                ByteRange { start, end: start + pattern_len }
            })
            .collect()
    };
...
mod tests {
    ...
    fn test_regenerate_structure() {
        let generator = Generator {
            base: vec![0xAA, 0xBB],
            repetition: RepetitionSpec { count: 3, stride: 4, start_offset: 0, byte_ranges: vec![] },
        };
